...
static const uint8_t TRAXXAS_sop_bind[] ={ 0x3C, 0x37, 0xCC, 0x91, 0xE2, 0xF8, 0xCC, 0x91 };
static const uint8_t TRAXXAS_sop_data[] ={ 0xA1, 0x78, 0xDC, 0x3C, 0x9E, 0x82, 0xDC, 0x3C };
//static const uint8_t TRAXXAS_sop_check[]={ 0x97, 0xE5, 0x14, 0x72, 0x7F, 0x1A, 0x14, 0x72 };

static const uint8_t TRAXXAS_init_vals[][2] = {
    //Init from dump
...
static void TRAXXAS_cyrf_bind_config()
{
    CYRF_ConfigSOPCode(TRAXXAS_sop_bind);
...
static void TRAXXAS_cyrf_data_config()
{
    CYRF_ConfigSOPCode(TRAXXAS_sop_data);
...
static void TRAXXAS_send_data_packet()
{
    packet[0] = 0x01;
    memset(&packet[1],0x00,TRAXXAS_PACKET_SIZE-1);
    // default channel order AETR
    //Steering
    uint16_t ch = convert_channel_16b_nolimit(Channels[3],500,1000);
    packet[2]=ch>>8;
    packet[3]=ch;
    //Throttle
    ch = convert_channel_16b_nolimit(Channels[2],500,1000);
    packet[4]=ch>>8;
    packet[5]=ch;
    //AUX3
    ch = convert_channel_16b_nolimit(Channels[0],500,1000);
    packet[6]=ch>>8;
    packet[7]=ch;
    //AUX4???
    ch = convert_channel_16b_nolimit(Channels[1],500,1000);
    packet[12]=ch>>8;
    packet[13]=ch;
...
{
    switch(cmd) {
        case PROTOCMD_INIT: TRAXXAS_init(0); return 0;
        case PROTOCMD_CHECK_AUTOBIND: return 0;  // Never Autobind
        case PROTOCMD_BIND:  TRAXXAS_init(1); return 0;
        case PROTOCMD_NUMCHAN: return 4;
        case PROTOCMD_DEFAULT_NUMCHAN: return 3;
        case PROTOCMD_CURRENT_ID: return Model.fixed_id;
        case PROTOCMD_MAX_ID: return 255;
        case PROTOCMD_RESET:
