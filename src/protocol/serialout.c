...
#endif
#define SERIAL_CHANNELS NUM_OUT_CHANNELS
static u8 packet[2 * SERIAL_CHANNELS + 5];
u8 num_channels;

static const char * const serial_opts[] = {
  _tr_noop("Protocol"), "SUMD", /*"S.BUS", CSRF*/ NULL,
  NULL
};
...
{
    build_data_pkt();
    UART_Send(packet, 2 * SERIAL_CHANNELS + 5);
#ifdef EMULATOR
    return 3000;
#else
    return 10000;
...
const void * SERIAL_Cmds(enum ProtoCmds cmd)
{
    switch(cmd) {
